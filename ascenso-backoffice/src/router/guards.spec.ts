...
function meResponse(permisos: string[] = []) {
  return { username: 'ana', permisos, destacamentoIds: [1], alcanceGlobal: false }
}
...
describe('authGuard', () => {
  ...
  it('sin token, refresh silencioso fallido: redirige a login con el destino original', async () => {
    ...
    const resultado = await authGuard.call(undefined, ruta({}, '/ninos/42'), from, next)

    expect(resultado).toEqual({ name: 'login', query: { redirect: '/ninos/42' } })
  })
  ...
  it('con permiso requerido y el usuario no lo tiene, redirige a forbidden', async () => {
    tokenService.set('token-existente')
    vi.mocked(authService.me).mockResolvedValue(meResponse(['NINOS_VER']))

    const resultado = await authGuard.call(undefined, ruta({ permission: 'ASISTENCIA_VER' }), from, next)
  ...
  it('con permiso requerido y el usuario sí lo tiene, deja pasar', async () => {
    tokenService.set('token-existente')
    vi.mocked(authService.me).mockResolvedValue(meResponse(['ASISTENCIA_VER']))
  ...
  it('no vuelve a llamar loadAuthorization si ya estaba cargada', async () => {
    tokenService.set('token-existente')
    vi.mocked(authService.me).mockResolvedValue(meResponse(['ASISTENCIA_VER']))
    usePermissionsStore().hydrate(['ASISTENCIA_VER'], [1], false)
